---
title: "Does Pay4Me Generate, Displace, or Transform Purchase Activity?"
...
```{python py-setup, include=FALSE, cache=FALSE}
import pandas as pd
import numpy as np
```
...
```{python data-loading-py, cache=FALSE}
...
```{r viz-1, fig.cap="Figure 1. Pay4Me share of daily events is stable across 11 days with no inverse relationship with self-purchase - inconsistent with displacement."}
df |>
  count(date, source) |>
...
```{r viz-2, fig.cap="Figure 2. Both channels peak in the evening - Pay4Me at 19:00, self-purchase at 18:00. The one-hour difference is statistically significant but practically small."}
df |>
...
```{r viz-3, fig.cap="Figure 3. Product category (L1) by channel. Self-purchase is internet-dominant (83%); Pay4Me has the highest voice share of all three channels (43%)."}
df |>
...
```{r viz-4, fig.cap="Figure 4. Pay4Me funnel: 68.9% reach product selection; 47.8% reach verification; 19.3% complete delivery. Each step requires a distinct user action."}
...
```{r viz-5, fig.cap="Figure 5. Session-length density by channel. Pay4Me sessions are shorter (mode = 3 steps) than self-purchase (mode = 4 steps), reflecting the fixed four-step funnel."}
df |>
